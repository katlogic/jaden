...
Compiler.prototype = {
  ...
  visitTag: function(tag){
    ...
    if (this.db) {
      var i, n, m, pat = "/\$\{([^}]+)\}/";
      for (i = 0; i < tag.block.nodes.length; i++) {
        n = tag.block.nodes[i];
        if ((n instanceof nodes.Text) && (m = n.val.match(pat))) {
          n.setAttribute('data-text', "'" + m[1] + "'", true);
          tag.block.nodes.splice(i, 1);
        }
      }
      for (i = 0; i < tag.attrs.length; i++) {
        n = tag.attrs[i]
        if (m = n.val.match(pat)) {
          tag.setAttribute("data-" + attr.name, "'" + m[1] + "'", true);
          tag.attrs.splice(idx, 1);
        }
      }
    }
